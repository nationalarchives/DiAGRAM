#' Load default model
#'
#' Load the default .bif model that should seed model scoring This file was
#' generated by the original authors.
#'
#' @return object of class bn.fit
#' @export
load_default_model = function() {
  fname = system.file("extdata", "model", "model.bif", package = "diagramLambda", mustWork = TRUE)
  bnlearn::read.bif(fname)
}

#' Score model
#'
#' Main entry point for scoring a model to be called by the exposed API
#' /score_model function. Should function appropriately for both simple and
#' advanced responses (as indicated by the class attribute of responses
#' parameter). Will call necessary preprocessing on the responses and model
...
#' Ensure grain model
#'
#' Ensure that model has type grain required for calculating score using the
#' {gRain} library.
#'
...
#' Convert node information to the same JSON format that would be received from
#' the advanced
#'
#' @param model format
