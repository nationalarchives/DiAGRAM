...
#'
#' Utility function for creating to_numeric functions
#' for each node in a response.
#'
#' @param class_name class attribute to give a single response
#' node that will be passed to the generic to_numeric
#' @return a function that takes a single response node, attaches
#' the class_name as a class and calls to_numeric
...
# To numeric funcs
#
# List of functions that will convert each response
# from a simple model to a numeric one according to the
# fixed weightings attributed to each response. List
# is named according to the node in the model that
# it represents.
...
#' To numeric
#'
#' Generic function for calling to_numeric on response nodes.
#' First calls the generic check() for assertion checking the recieved
#' response value.
#'
...
# res character vector of length 10
#' @export
...
# res length 1 character vector, see get_option_val.physical_disaster for allowed values
#' @export
...
# res a named list of 4 responses, each response is length 1.
#' @export
...
# res numeric vector of length 3
#' @export
...
# res a named list of 3 responses with the following
# length character vectors c(1, 1, 2)
#' @export
...
# Max score to either question (100, Yes) gives a final score of 100,
# if (x, No) return x
# res a named list of 2 responses: structure list(numeric(1), character(1)).
#' @export
...
# res a named list of 2 responses with length 1 numeric vectors.
#' @export
...
#
# res a tibble with class simple_responses
...
# res a list object with a simple part and the advanced part
#' @export
to_numeric.advanced_responses = function(res) { # nolint
  simple = to_numeric(res$data$simple)
  # want to leave advanced nodes untouched as already in probability
  # format, just pass them on
  out = list(
