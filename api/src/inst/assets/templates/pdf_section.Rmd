...
question_data = diagramAPI::get_questions_tibble()
base_model = diagramAPI::get_base_model(req)
policies = diagramAPI::find_scenario_diffs(req)
comments_data = diagramAPI::get_comments(req)
scenario_names = diagramAPI::get_scenario_names(req)
...
if (is_advanced) {
  ...
  # diagramAPI::join_questions_responses(question_data, simple_base)
} else {
  diagramAPI::join_questions_responses(question_data, base_model) %>%
    diagramAPI::pdf_table() %>%
...
if (any_policies) {
  purrr::walk(policies, function(x) {
    ...
    if (nrow(x$data) > 0) {
      ...
      diagramAPI::join_questions_responses(question_data, x) %>%
        diagramAPI::diff_table() %>%
        knitr::kable(format = "latex", longtable = TRUE, booktabs = TRUE) %>%
        kableExtra::kable_styling(full_width = TRUE, font_size = 7) %>%
        cat()
    }
