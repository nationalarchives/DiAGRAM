...
The {lambdr} package, along with the backend application code (see `src/`) and its dependencies, are
installed into a container image (see `Dockerfile`), which is then deployed as a Lambda function.
